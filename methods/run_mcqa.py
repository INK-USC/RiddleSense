...
class DataTrainingArguments:
    ...
    val_file: str = field(metadata={"help": "Should contain the data files for the task."})
    ...
    num_choices: int = field(metadata={"help": "Should contain the data files for the task."})
...
def main():
    ...
    try:
        ...
        label_list = processor.get_labels(num_choices=data_args.num_choices)
    ...
    if training_args.do_predict:
        ...
        logits = np.array(pred_result.predictions)
        with open(data_args.prediction_output, "wb") as f:
            print(logits.shape)
            logger.info("Writing prediction logits to %s", f.name)
